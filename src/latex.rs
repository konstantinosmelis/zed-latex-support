...
impl LatexExtension {
    fn language_server_binary_tree(
        &mut self,
        language_server_id: &zed::LanguageServerId,
        worktree: &zed::Worktree
    ) -> zed::Result<String> {
        if let Some(path) = worktree.which("texlab") {
            return Ok(path);
        }
...
impl zed::Extension for LatexExtension {
    ...
    fn language_server_command(
        ...
        Ok(zed::Command {
            command: self.language_server_binary_tree(language_server_id, worktree)?,
